...
private:
    void tryReplaceAndEqualsChainsWithConstant(QueryTreeNodePtr & node)
    {
        ...
        for (const auto & argument : function_node.getArguments())
        {
            ...
            else
            {
                ...
                const auto & not_equals_arguments = argument_function->getArguments().getNodes();
                const auto & lhs = not_equals_arguments[0];
                const auto & rhs = not_equals_arguments[1];
        ...
        if (and_operands.size() == 1)
        {
            ...
            // happens when there is an `... OR true|false ...`
            if (and_operands[0]->getResultType()->equals(*function_node.getResultType()))
            {
                node = std::move(and_operands[0]);
                return;
            }

            /// otherwise add a stub 1 to make AND correct
            and_operands.push_back(std::make_shared<ConstantNode>(static_cast<UInt8>(1), function_node.getResultType()));
        }
    ...
    {
        auto & function_node = node->as<FunctionNode &>();
        // is this a single `a = 1`, or the whole cond i.e. `a = 1 OR a = 2`
        assert(function_node.getFunctionName() == "or");

        // bcos there might be other expr that are not part of the optimization.
        // i.e. WHERE user_id=101 OR user_id=102 OR user_id=103 OR message='b'
        // becomes WHERE (message = 'b') OR (user_id IN (101, 102, 103))
        QueryTreeNodes or_operands;
        ...
        // runs n times, for every `user_id = ...`
        for (const auto & argument : function_node.getArguments())
        {
            auto * argument_function = argument->as<FunctionNode>();
            ...
            const auto & equals_arguments = argument_function->getArguments().getNodes();
            const auto & lhs = equals_arguments[0]; // 
            const auto & rhs = equals_arguments[1]; // individual: 101, 102, 103
            ...
            if (const auto * lhs_literal = lhs->as<ConstantNode>();
                ...
                add_equals_function_if_not_present(rhs, lhs_literal);
                     ...
                     rhs_literal && !rhs_literal->getValue().isNull())
                // rhs_literal->getValueStringRepresentation() = 101
        ...
        // 1: message = 'b' | 2: user_id IN (101, 102, 103)
        for (auto & [expression, equals_functions] : node_to_equals_functions)
        {
            const auto & settings = getSettings();
            // if chain size smaller than optimize_min_equality_disjunction_chain_length (defaults to 3) AND not low cardinality, dont optimize.
            ...
            auto rhs_node = std::make_shared<ConstantNode>(std::move(args)); // (101, 102, 103)
        ...
        // means no optimization can be done.
